...
*/
exports.buildProducer = function(Kafka, producer_opts, shutdown, socket, runOpts) {
    // Create Kafka producer
	console.log(`runOpts: ${JSON.stringify(runOpts)}`);
	var topicName = runOpts.kpi;
	console.log(`runOpts topicName: ${JSON.stringify(runOpts.kpi)}`);
    ...
    producer.on('delivery-report', function(err, dr) {
        if (err) {
            ...
            console.log('Message produced, offset: ' + dr.offset + ' ' +dr.value);
            socket.emit('publish',{msg: 'Message produced, offset: ' + dr.offset + ' ' +dr.value});
			
        }
    });

    function sendMessages(counter, topic, partition, runOpts) {
        	...
        	setTimeout(function () {
            	if (producer.isConnected()) {
                	sendMessages(counter, topic, partition, runOpts);
            	}
        	}, timeout);
    }
    ...
    producer.on('ready', function() {
        ...
        sendMessages(counter, topic, 0, runOpts);
