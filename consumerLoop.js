 ...
 */
exports.buildConsumer = function(Kafka, consumer_opts, topicName, shutdown, socket, runOpts) {
...
//            topic: topicName,
 ...
 //                   console.error('ERROR - Topic ' + topicName + ' does not exist. Exiting');
                    console.error('ERROR - Topic ' + runOpts.kpi + ' does not exist. Exiting');
                    shutdown(-1);
                }
            }
        });
        ...
        consumerLoop = setInterval(function () {
            if (consumedMessages.length === 0) {
                ...
                for (var i = 0; i < consumedMessages.length; i++) {
					...
					socket.emit('subscribe',{msg: 'Message consumed: topic=' + m.topic + ', partition=' + m.partition + ', offset=' + m.offset + ', key=' + m.key + ', value=' + m.value.toString()});
